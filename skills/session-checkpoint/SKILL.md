...
| | Path | Lifetime |
| --- | --- | --- |
| **Checkpoint** | `sessions/YYYY-MM-DD-HHmm-<slug>.md` | Superseded by the next checkpoint |
| **Decision record** | `sessions/decisions/YYYY-MM-DD-<slug>.md` | Stands until a later decision supersedes it |
...
1. **Ground every fact in a tool call, not in memory.** Date, hostname, branch, HEAD commit, working-tree cleanliness — read them from the shell *in this session*, immediately before writing. A checkpoint whose commit hash is wrong is worse than no checkpoint: `session-resume` trusts these fields to detect drift.
...
4. **Redact.** No API keys, tokens, passwords, connection strings, internal hostnames beyond what git already exposes, or personal data. For environment variables, record the **name only** — never the value. These files are committed to a repository that other people may read.
...
date "+%Y-%m-%d-%H%M"                  # filename prefix
hostname -s                            # machine label
```
...
Path: `<repo-root>/sessions/YYYY-MM-DD-HHmm-<slug>.md`
...
- **One file per checkpoint. Never append to or edit an existing one.** Two machines writing distinct filenames never conflict in git; two machines editing the same file conflict every time.
...
- **Dates, not sequence numbers.** Numbered ADRs (`001-`, `002-`) require a central allocator. Two machines offline both reach for `004-` and collide on the next pull.
...
Multiple decisions on one day are fine — the slug distinguishes them.
