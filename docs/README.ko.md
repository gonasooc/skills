...
직접 설치한 스킬은 이름으로 호출합니다 — Claude Code에서는 `/interviewer`, Codex에서는 `$interviewer`. 제품의 스킬 피커를 사용해도 됩니다. Claude Code 플러그인으로 설치하면 네임스페이스가 붙으므로 `/gonasooc-skills:interviewer`로 호출합니다. `interviewer`는 자연어로 요청해도 실행되지만, `session-checkpoint`와 `session-resume`은 직접 요청하거나 명시적으로 호출했을 때만 동작합니다. `agents/openai.yaml` 같은 에이전트 전용 파일은 이를 사용하지 않는 에이전트에서는 무시됩니다.
...
```
sessions/
├── 2026-07-28-143215-auth-retry.md    # 한 시점 — 다음 체크포인트가 대체
└── decisions/
    └── 2026-07-28-143208-token-in-memory.md  # 제약 — 뒤집히기 전까지 유효
```
...
두 디렉터리 모두 추가만 하고 고치지 않습니다. 기록에 고전적인 ADR처럼 순번을 매기지 않고 초 단위 시각을 쓰는 이유도 같습니다 — 순번은 중앙 발급자가 필요한데, 두 기기가 각자 오프라인에서 `004-`를 집을 수 있습니다. 쓰기 직전 존재 여부를 확인해 로컬 덮어쓰기를 막고, 타임스탬프로 기기 간 파일명 충돌 가능성을 낮춥니다(완전히 없다고 주장하지는 않습니다). 대체는 새 기록에 거는 단방향 링크로 표현하고, 옛 파일은 건드리지 않습니다.

둘 다 **명시적 호출 전용**입니다. 각 스킬은 호출 게이트로 시작하며, 주변 대화만 보고 에이전트가 추론해 불러온 경우에는 아무 파일도 쓰지 않고 멈춥니다. Codex는 `allow_implicit_invocation: false`로 호스트 수준에서도 이를 막습니다. 에이전트가 알아서 "이 세션은 기록할 만하다"고 판단하는 일은 없습니다 — 남기기로 결정한 세션만 `sessions/`에 들어갑니다.
