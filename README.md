...
Invoke a directly installed skill by name — `/interviewer` in Claude Code or `$interviewer` in Codex — or use the product's skill picker. Claude Code namespaces plugin skills, so the plugin form is `/gonasooc-skills:interviewer`. `interviewer` also responds to a plain natural-language request. Agent-specific files like `agents/openai.yaml` are ignored by agents that don't use them.
